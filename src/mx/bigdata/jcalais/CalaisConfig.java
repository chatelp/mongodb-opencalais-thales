...
public final class CalaisConfig {
	...
	public enum ProcessingParam {
		/**
		 * Format of the input content (see details below) Valid values are:
		 * "TEXT/XML", "TEXT/HTML", "TEXT/HTMLRAW", "TEXT/RAW"
		 */
		 ...
		 * (RDF) and/or SocialTags Valid values are: "GenericRelations" - to
		 * enable Generic Relations; "SocialTags" - to enable Social Tags (could
		 * combine comma-separated options: "GenericRelations,SocialTags"
		 */
		...
		/**
		 * Indicates whether entire XML/RDF document is saved in the Calais
		 * Linked Data repository Valid values are: true, false
		 */
	...
	private static final ImmutableMap<ConnParam, Integer> CONNECTION_DEFAULTS = new ImmutableMap.Builder<ConnParam, Integer>()
			.put(ConnParam.CONNECT_TIMEOUT, 90000)
			.put(ConnParam.READ_TIMEOUT, 90000).build();

	private static final ImmutableMap<String, String> PROCESSING_DEFAULTS = new ImmutableMap.Builder<String, String>()
			.put("contentType", "TEXT/RAW")
			.put("outputFormat", "application/json")
			.put("enableMetadataType", "GenericRelations,SocialTags")
			.put("calculateRelevanceScore", "true").build();

	private static final ImmutableMap<String, String> USER_DEFAULTS = new ImmutableMap.Builder<String, String>()
			.put("externalID", UUID.randomUUID().toString())
			.put("submitter", SUBMITTER).build();

	private final Map<String, String> processingDirectives = Maps
			.newHashMap(PROCESSING_DEFAULTS);

	private final Map<String, String> userDirectives = Maps
			.newHashMap(USER_DEFAULTS);

	private final Map<ConnParam, Integer> connectionDirectives = Maps
			.newHashMap(CONNECTION_DEFAULTS);
